...
def transcribe_every_video(
    ...
    video_files = [
        os.path.join(dir_name, f) for f in video_files if f.endswith(".mp4")
    ...
    for i, video in enumerate(video_files):
        ...
        transcriptions[os.path.basename(video)] = (
            transcription  # store transcription in dictionary
        )
...
def create_labeled_dataset(
    ...
    assert (
        "filename" in labels_df.columns and "label" in labels_df.columns
    ), "Labels file must contain 'filename' and 'label' columns"

    # Check for mismatched videos between labels and transcriptions, sets are used to ignore order and duplicates
    unique_videos_in_transcriptions = set(
        labels_df["filename"].apply(lambda x: os.path.basename(x)).unique().tolist()
    )
    unique_videos_in_labels = set(
        [os.path.basename(video) for video in transcriptions.keys()]
    )
    if unique_videos_in_transcriptions != unique_videos_in_labels:
        if unique_videos_in_transcriptions.issubset(unique_videos_in_labels):
            warnings.warn(
                "Some videos in the labels file are missing in the transcriptions."
            )
        elif unique_videos_in_labels.issubset(unique_videos_in_transcriptions):
            warnings.warn(
                "Some videos in the transcriptions are missing in the labels file."
            )
        else:
            raise ValueError(
                "Mismatch between videos in labels file and transcriptions."
            )
